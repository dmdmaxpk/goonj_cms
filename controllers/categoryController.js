...
exports.get_cats = async (req, res) => {
	...
	try {
		// GET: Video Service
		let {data} = await axios.get(`${config.videoServiceUrl}/category`);
		res.render('category_main', {title: 'Category', data});		// Rendering the cateogory_main view and passing the categories list
	}
	catch (err) {
...
exports.add_cat = async (req, res) => {
	...
	try{
		// POST: Video Service
		let {data} = await axios.post(`${config.videoServiceUrl}/category`, postData);
		console.log(data);
	}
	catch (err) {
		console.error(err.code);
	...
	res.redirect('/category');		// Redirecting to main page
}
...
exports.delete = async (req, res) => {
	...
	try {
		...
		let {data} = await axios.delete(`${config.videoServiceUrl}/category?cat_id=${cat_id}&subcat_id=${subcat_id}`);
		res.send(data);
	}
	catch (err) {
		console.error(err.code);
